...
class CarsErrorResolver {
  constructor(
    private readonly manufacturerNotFoundErrorResolver: ManufacturerNotFoundErrorResolver
  ) {}
  resolveCarsError(exception: any): any {
    const manufacturerId = this.manufacturerNotFoundErrorResolver.isManufacturerNotFoundError(
      exception
    );
    if (manufacturerId) {
      return new BadRequestException(
        `No manufacturer found with given id [${manufacturerId}]`
      );
    }
    return undefined;
  }
...
class SqlManufacturerNotFoundErrorErrorResolver
  implements ManufacturerNotFoundErrorResolver {
  isManufacturerNotFoundError(exception: any): string {
...
export class CarsExceptionFilter extends BaseExceptionFilter {
  ...
  constructor(private readonly connection: Connection) {
    ...
    switch (connection.driver.options.type) {
      case 'mongodb': // TODO
      default:
        this.errorResolver = new CarsErrorResolver(
